...
import wandb
wandb.init(project="cp2", config={"nonlinearity": "ReLU",
...
def main():
    ...
    do_run_gui = len(args.dir)==0 and len(args.image_path)==0
    if do_run_gui:
        if args.add_model:
    ...
    if args.pretrained_model is None or args.pretrained_model.lower() in ['none', 'false', '0']:
    ...
    if pretrained_model and not os.path.exists(pretrained_model):
        ...
        if model_type=='nuclei':
            ...
            szmean = 30.
    builtin_size = model_type in ['cyto', 'cyto2', 'nuclei']
